...
const App: React.FC = () => {
  ...
  const [wordCount, setWordCount] = useState<string>('1200'); // Default ~8 min
  ...
  const [lengthType, setLengthType] = useState<'words' | 'duration'>('duration');
  const [videoDuration, setVideoDuration] = useState<string>('8'); // Default 8 min

  // New News Params
  const [audienceAge, setAudienceAge] = useState<AudienceAge>('Millennials');
  const [contentFocus, setContentFocus] = useState<ContentFocus>('General_News');
  ...
  // Set default model to Gemini 3 (index 0)
  ...
  const handleGenerateScript = useCallback(async () => {
    if (!title.trim()) {
      setError('Vui lòng nhập hoặc chọn một tiêu đề tin tức.');
      return;
    }
    setIsLoading(true);
    setError(null);
        ...
        <div className="flex-1 text-center">
            <a href="/" className="inline-flex justify-center items-center gap-3 no-underline transition-opacity hover:opacity-80" style={{ textDecoration: 'none' }}>
              ...
              <h1 className="text-2xl font-bold text-accent">
                Trợ lý Sáng tạo Tin tức Thái Lan
              </h1>
            </a>
            <p className="text-text-secondary mt-1 text-sm md:text-base">
              Tạo kịch bản tin tức chuẩn văn hóa, trung lập cho thị trường Thái Lan.
            </p>
        </div>
